...
export default class AppDB {
    ...
    public initAppSettingsTable() {
        this.db!.exec(`
            CREATE TABLE IF NOT EXISTS AppSettings (id INTEGER PRIMARY KEY, name TEXT, json JSONB);
            INSERT INTO AppSettings (id, name, json) VALUES (1, 'settings', '{}') ON CONFLICT (id) DO NOTHING;
        `);
    }

    public async setSettingsValue(key: string, value: any) {
        return await this.db!.exec(`
            UPDATE AppSettings SET json = jsonb_set(json, '{${key}}', to_jsonb('${value}'::text), true) WHERE id = 1;
        `,);
    }

    public async getSettingsValue<T>(key: string): Promise<T | null> {
        const res = await this.db!.query(`
            SELECT json->'${key}' AS value FROM AppSettings WHERE id = 1;
        `);
    ...
    public async getAllSettings(): Promise<Object | null> {
        const res = await this.db!.query(`
            SELECT json AS value FROM AppSettings WHERE id = 1;
        `);
        return res.rows[0]? res.rows[0].value! : null;
    }
